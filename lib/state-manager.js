...
class StateManager {
  ...
  logBlocker(blocker) {
    ...
    // Also save blocker to separate file
  ...
  /**
   * Resume a feature from history
   */
  resumeFeature(featureId) {
    const historyPath = path.join(this.historyDir, `${featureId}.json`);

    if (!fs.existsSync(historyPath)) {
      throw new Error(`Feature ${featureId} not found in history`);
    }
